<template>
  ...
  >
    <section class="flex h-[min(88vh,820px)] w-full max-w-6xl flex-col overflow-hidden rounded-[32px] border border-slate-200/80 bg-white shadow-2xl">
      <header class="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 px-6 py-5">
        <div>
          <div class="flex items-center gap-3">
            <span class="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-600">{{ marketLabel }}</span>
            <h3 class="text-2xl font-semibold text-slate-900">{{ title }}</h3>
          </div>
          <p class="mt-2 text-sm text-slate-500">{{ symbol || '--' }}</p>
        </div>
        ...
        <div class="flex items-center gap-3">
          <div class="flex flex-wrap gap-2 rounded-full border border-slate-200 bg-slate-50 p-1">
            <button
              ...
              :class="timeframe === item ? 'bg-slate-900 text-white' : 'text-slate-600 hover:bg-white'"
