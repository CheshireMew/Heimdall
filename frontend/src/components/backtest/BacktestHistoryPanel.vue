<template>
  <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6 overflow-hidden flex flex-col min-h-[320px] transition-colors">
    <div class="flex items-center justify-between gap-3 mb-4">
      <div>
        <h2 class="text-xl font-bold text-gray-900 dark:text-white">
          {{ page.historyMode === 'paper' ? $t('backtest.paperRecent') : $t('backtest.recent') }}
        </h2>
        <div class="text-xs text-gray-500 dark:text-gray-400">
          {{ page.historyMode === 'paper' ? $t('backtest.paperRecentHint') : (page.enableHistoryCompare ? $t('backtest.selectForCompareHint') : $t('backtest.historyBrowseHint')) }}
        </div>
      </div>
      <div class="inline-flex rounded-lg border border-gray-200 dark:border-gray-700 p-1 bg-gray-50 dark:bg-gray-900/40">
        <button
          class="tab-btn"
          :class="{ active: page.historyMode === 'backtest' }"
          @click="page.historyMode = 'backtest'"
        >
          {{ $t('backtest.recent') }}
        </button>
        <button
          class="tab-btn"
          :class="{ active: page.historyMode === 'paper' }"
          @click="page.historyMode = 'paper'"
        >
    ...
    <div class="overflow-y-auto flex-1 space-y-2">
      <div
        v-for="run in page.visibleHistory"
        ...
        @click="page.openRunDetail(run, page.historyMode)"
      >
        <div class="flex gap-3">
          <label v-if="page.enableHistoryCompare && page.historyMode === 'backtest' && run?.id" class="pt-1" @click.stop>
          ...
          <div>
            ...
            <div class="text-xs text-gray-400 mt-1">
              {{ formatDateTime(run?.created_at) }} · {{ page.runStatusLabel(run) }}
            </div>
          </div>
        </div>
        <div class="text-right">
          ...
          <button
            v-if="page.historyMode === 'paper' && run?.status === 'running' && run?.id"
            class="mt-2 inline-flex items-center rounded-md bg-red-50 px-2 py-1 text-xs font-semibold text-red-600 hover:bg-red-100 dark:bg-red-950/40 dark:text-red-300"
            @click.stop="page.stopPaperRun(run.id)"
          >
