...
const createEmptyMarketData = (): DcaMarketState => ({
  rsi: null,
  sentiment: null,
  sentimentLabel: '',
})
...
const normalizeMarketData = (value: unknown): DcaMarketState => {
...
export function useDcaPage() {
  ...
  const marketData = reactive(normalizeMarketData(restoredSnapshot?.marketData))
