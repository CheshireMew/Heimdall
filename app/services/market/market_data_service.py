...
class MarketDataService:
    ...
    def load_ohlcv_range(
        ...
        try:
            try:
                ...
                rows = self._fetch_gap(symbol, timeframe, start_ts, end_ts_exclusive)
    ...
    def load_cached_ohlcv_range(
        ...
        range_start = time.time()
        try:
            rows = self.kline_store.get_range(self._storage_symbol(symbol), timeframe, start_ts, end_ts)
            return self._range_result(
                rows=rows,
                timeframe=timeframe,
                start_ts=start_ts,
    ...
    def load_live_ohlcv_range(
        ...
        range_start = time.time()
        try:
            rows = self._fetch_gap(symbol, timeframe, start_ts, end_ts + 1)
            return self._range_result(
                rows=rows,
                timeframe=timeframe,
                start_ts=start_ts,
