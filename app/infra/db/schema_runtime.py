...
from app.infra.db.database import current_database_url, get_database_runtime
...
def prepare_db() -> None:
    ...
    _version_unmanaged_database(alembic_config)
...
def verify_database_schema() -> None:
    ...
    with get_database_runtime().engine.connect() as connection:
        migration_context = MigrationContext.configure(
            connection,
            opts={"compare_type": True},
        )
        current = migration_context.get_current_revision()
        diffs = _relevant_schema_diffs(compare_metadata(migration_context, Base.metadata))
    if current != expected:
        raise RuntimeError(
    ...
    _assert_model_tables_present()


def _relevant_schema_diffs(diffs: list[object]) -> list[object]:
    return [diff for diff in _flatten_schema_diffs(diffs) if not _is_ignored_schema_diff(diff)]
...
def _is_ignored_schema_diff(diff: object) -> bool:
    if not isinstance(diff, tuple) or not diff:
        return False
    if get_database_runtime().engine.dialect.name != "sqlite" or diff[0] != "modify_nullable":
        return False
...
def _alembic_config():
    ...
    config.set_main_option("sqlalchemy.url", current_database_url())
    config.attributes["use_configured_database_url"] = True
    return config


def _version_unmanaged_database(alembic_config) -> None:
    from alembic import command

    inspector = inspect(get_database_runtime().engine)
...
def _assert_model_tables_present() -> None:
    inspector = inspect(get_database_runtime().engine)
    missing = [
        table.name
        for table in Base.metadata.sorted_tables
