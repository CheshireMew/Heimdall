...
from app.router_service_ports import BinanceWeb3RwaPort
from app.runtime_refs import MARKET_BINANCE_WEB3_RWA
from app.contracts.dto.binance.rwa import (
...
binance_web3_rwa_dependency = runtime_dependency(MARKET_BINANCE_WEB3_RWA)
...
async def get_binance_rwa_symbols(
    ...
    service: BinanceWeb3RwaPort = Depends(binance_web3_rwa_dependency),
):
    return await service.list_symbols(platform_type=platform_type)
...
async def get_binance_rwa_meta(
    ...
    return await service.get_meta(chain_id=chain_id, contract_address=contract_address)
...
async def get_binance_rwa_asset_market_status(
    ...
    return await service.get_asset_market_status(chain_id=chain_id, contract_address=contract_address)
